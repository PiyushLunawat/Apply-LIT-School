...
const StatusMessage: React.FC<StatusMessageProps> = ({ student }) => {
  ...
  const [intervalId, setIntervalId] = useState<ReturnType<
    typeof setInterval
  > | null>(null); // Use ReturnType<typeof setInterval>
  ...
  useEffect(() => {
    switch (applicationStatus) {
      case "under review":
        setHeadMessage(
          `Hey ${student?.firstName}, your application is under review...`
        );
        setSubMessage(
      ...
      case "on hold":
        setHeadMessage(
          `Hey ${student?.firstName}, your application review has been put on hold...`
        );
        setSubMessage(
      ...
      case "interview cancelled":
        setHeadMessage(
          <>
            <span className="text-[#00A3FF]">Congratulations!</span> Your
      ...
      case "rejected":
        setHeadMessage(
          <>
            Hey {student?.firstName}, we’re very sorry but we{" "}
            <span className="text-[#FF503D]">cannot move forward</span> with
            your application.
          </>
        );
        setSubMessage(
      ...
      case "selected":
        setHeadMessage(
          <>
            <span className="text-[#00A3FF]">Congratulations!</span> on clearing
            the interview!
          </>
        );
        setSubMessage(
          ...
          } Cohort Scheduled for ${new Date(
            cohortDetails?.startDate
          ).toDateString()} by completing the payment of your reservation fee.`
        );
        break;
      case "waitlist":
        setHeadMessage(
          `Hey ${student?.firstName}, you have been put on the waitlist.`
        );
        setSubMessage(
          `Thank you for taking the counselling interview. You have been waitlisted. You will be updated with regards to your seat approval before ${new Date(
            cohortDetails?.startDate
          ).toDateString()}.`
        );
        break;
      case "not qualified":
        setHeadMessage(
          <>
            Hey {student?.firstName}, you have{" "}
            <span className="text-[#FF503D]">not qualified</span> for the
            upcoming {cohortDetails?.programDetail?.name} Cohort.
          </>
        );
        setSubMessage(
  ...
  useEffect(() => {
    if (applicationDetails?.updatedAt) {
      const targetTime =
        new Date(applicationDetails?.updatedAt).getTime() + 24 * 60 * 60 * 1000; // Add 24 hours to the provided time
  ...
  return (
    <>
      <div
        className={`bg-transparent flex relative ${
          ["interview scheduled", "interview concluded"].includes(
            applicationStatus
          )
            ? "h-[300px] sm:h-[450px]"
            : "h-[250px] sm:h-[350px]"
        ...
        <div className="w-full md:w-[850px] mx-auto flex flex-col gap-4 sm:gap-8 justify-center items-center">
            ...
            <div className="space-y-2">
              ...
              {countdown === "00:00:00" && (
                <div className="text-[#FF503D]">
                  A reminder has been sent to our team, we'll review it shortly
                </div>
              )}
            </div>
          )}
          ...
          {["interview scheduled", "interview concluded"].includes(
            ...
            <div className="mx-8 sm:mt-4 sm:mx-16 text-xs sm:text-sm md:text-base text-center font-normal sm:space-y-3">
              ...
              <Button
                className="bg-[#FFFFFF2B] hover:bg-[#FFFFFF]/30 rounded"
