          ...
          >
            <div className="flex items-center text-base font-normal space-x-3">
              <Badge size={"icon"} className="bg-[#1B1B1C] rounded-lg">
                {item.type === "file" && (
                  <File className="text-white w-4 h-4" />
                )}
                {item.type === "image" && (
                  <Image className="text-white w-4 h-4" />
                )}
                {item.type === "video" && (
                  <SquarePlay className="text-white w-4 h-4" />
                )}
                {item.type === "link" && (
                  <MousePointer2 className="text-white w-4 h-4" />
                )}
              </Badge>
              <span className="w-[200px] sm:w-[300px] truncate">
                {item.name ||
                  `${process.env.NEXT_PUBLIC_AWS_RESOURCE_URL}/${item.url}`}
              </span>
            </div>
            <Badge size={"icon"} className="bg-[#1B1B1C] rounded-lg">
              {item.type !== "link" ? (
                <a
                  href={`${process.env.NEXT_PUBLIC_AWS_RESOURCE_URL}/${item.url}`}
                  download
