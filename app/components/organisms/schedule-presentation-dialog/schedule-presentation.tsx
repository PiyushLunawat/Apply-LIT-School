...
export function SchedulePresentation({
  ...
  const handleScheduleRedirect = () => {
    if (!selectedInterviewer || !student?._id || !latestCohort?.cohortId?._id)
      return;
    const url = `https://cal.litschool.in/${selectedInterviewer?.personalUrl}/${
      selectedInterviewer?.events[0]?.eventName || ""
    }?name=${student?.firstName || ""}${student?.lastName || ""}&email=${
      student?.email || ""
      ...
      <div className="grid gap-3">
        ...
        <div className="space-y-1">
          ...
          <div className="flex gap-4 h-5 items-center">
            ...
            <p className="text-sm text-muted-foreground">
              {eventCategory === "Application Test Interview"
                ? `Submitted application on ${new Date(
                    latestCohort?.applicationDetails?.createdAt
                  ).toLocaleDateString()}`
                : `Submitted on ${new Date(
                    latestCohort?.litmusTestDetails?.createdAt
                  ).toLocaleDateString()}`}
            </p>
          </div>
        </div>
            ...
            <div className="flex flex-col items-start space-y-2">
              {interviewer.length > 0 ? (
                <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {interviewer
                    .filter(
                      (interviewer: any) => interviewer?.events?.length > 0
                    )
