...
const InputOTPGroup = React.forwardRef<React.ElementRef<'div'>, React.ComponentPropsWithoutRef<'div'>>(({ className, ...props }, ref) => (
  <div ref={ref} className={cn('flex items-center gap-1.5 sm:gap-2.5', className)} {...props} />
))
  ...
  ({ index, className, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn(
          'relative flex h-[44px] w-[44px] sm:h-[52px] sm:w-[60px] items-center justify-center border border-[#ffffff]/[0.2] text-sm sm:text-base font-semibold shadow-sm transition-all rounded-xl bg-transparent',
          isActive && 'z-10 border-white',
          className,
        )}
        {...props}
      >
