  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      try {
        ...
        const { generateIDCardPDF } = await import("~/utils/pdf-generator");
        await generateIDCardPDF(student || details);
          ...
          <div className="relative flex items-center gap-4">
            <div className="relative rounded-xl bg-white group w-16 h-16">
              <img
                ...
                } mx-auto bg-white py-1`}
              />
              {student?.bloodGroup && student?.profileUrl ? (
                <button
                  className="absolute rounded-xl inset-0 bg-black/80 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
