...
export default function LitmusTest({ student }: LitmusTestProps) {
  ...
  useEffect(() => {
    ...
    if(latestCohort?.litmusTestDetails?.status === 'completed' && !latestCohort?.feedbackDetails){
  ...
  <div className='flex flex-col items-start bg-[#09090B] text-white w-full mx-auto px-4 py-8 sm:p-[52px] space-y-4 sm:space-y-8'>
      ...
      <div className='space-y-8'>
        <div className={`${status === 'completed' ? 'bg-[#64748B1F] border p-6 flex gap-6 rounded-xl' : ''}`}>
          ...
          <div className='hidden md:flex w-[82px] h-[82px] bg-[#1388FF1F] flex items-center justify-center rounded-full'>
              <img src='/assets/icons/congratulation-icon.avif' className='w-8' />
            </div>
          }
              ...
              <div className='flex gap-4 items-center text-2xl font-medium'>
                {status === 'completed' &&
                  <div className='flex md:hidden w-[72px] h-[72px] bg-[#1388FF1F] items-center justify-center rounded-full'>
                    <img src='/assets/icons/congratulation-icon.avif' className='w-6' />
                  </div>
                }
    ...
    {status === 'completed' ? 
      <div className='space-y-6'>
        <div className="flex flex-col lg:flex-row">
          ...
          <div className={`!bg-transparent w-full min-h-[300px] py-4 flex flex-col gap-2 px-4 sm:px-10 rounded-2xl border ${getBadgeColor(litmusTestDetails?.scholarshipDetail?.scholarshipName)}`}>
            <div className='flex flex-col my-auto gap-y-4 sm:gap-y-12'>
              ...
              <Button size={'xl'} className={`w-full sm:w-fit mt-4 !text-white ${getBadgeColor(litmusTestDetails?.scholarshipDetail?.scholarshipName)}`}
        ...
        <Card className='px-6 py-8 space-y-8'>
          <div className='flex justify-between items-center'>
            <div className="text-xl sm:text-2xl font-medium">LITMUS Challenge Review</div>
    ...
    <Dialog open={feedbackOpen} >
      ...
      <DialogContent className="border mx-auto max-h-[70vh] sm:max-h-[90vh] p-6 sm:p-8 overflow-y-auto max-w-[90vw] sm:max-w-5xl rounded-3xl">
        <LitmusFeedbackForm student={student} setClose={() => setFeedbackOpen(false)} />
      </DialogContent>
    </Dialog>
  </div>
  );
