...
import { CheckCircle, Clipboard, ClipboardCheck, Instagram, Linkedin, Mail, Minus, Phone, SaveIcon } from "lucide-react"
...
const formSchema = z
    ...
    .refine((data) => !data.applicationData.isExperienced || data.applicationData.experienceType, {
      message: "Experience Type is required.",
      path: ["applicationData.experienceType"],
    })
    .refine((data) => !data.applicationData.isExperienced || data.applicationData.jobDescription, {
    ...
    .refine((data) => data.applicationData.emergencyContact !== data.studentData.contact, {
      message: "Emergency contact and your contact must be different.",
      path: ["applicationData.emergencyContact"],
    })
    .refine((data) => data.applicationData.fatherContact !== data.studentData.contact, {
      message: "Father's contact and your contact must be different.",
      path: ["applicationData.fatherContact"],
    })
    .refine((data) => data.applicationData.motherContact !== data.studentData.contact, {
      message: "Mother's contact and your contact must be different.",
      path: ["applicationData.motherContact"],
    })
    .refine(
        (data) =>
            data.applicationData.fatherContact !== data.applicationData.motherContact || !data.applicationData.fatherContact,
        {
          message: "Father's contact and mother's contact must be different.",
          path: ["applicationData.motherContact"],
        },
    )
    .refine((data) => data.applicationData.fatherEmail !== data.studentData.email, {
      message: "Father's email and your email must be different.",
      path: ["applicationData.fatherEmail"],
    })
    .refine((data) => data.studentData.email !== data.applicationData.motherEmail, {
      message: "Mother's email and your email must be different.",
      path: ["applicationData.motherEmail"],
    })
    .refine(
        (data) =>
            data.applicationData.fatherEmail !== data.applicationData.motherEmail || !data.applicationData.fatherEmail,
        {
...
const ApplicationDetailsForm: React.FC = () => {
  ...
  const minGraduationDate = new Date(today.getFullYear() - 50, today.getMonth(), today.getDate())
      .toISOString()
      .split("T")[0]
  ...
  const updateFilteredCohorts = useCallback(
      (programId: string) => {
        ...
        const matching = openCohorts.filter((cohort) => cohort.programDetail === programId)

        console.log("Filtered cohorts for program", programId, ":", matching.length)
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const [programsData, centresData, cohortsData] = await Promise.all([getPrograms(), getCentres(), getCohorts()])
        ...
        const uniquePrograms = Array.from(new Set(open.map((cohort: any) => cohort.programDetail)))
  ...
  const handleCourseChange = useCallback(
      (value: string) => {
        ...
        if (currentCohort) {
          const currentCohortData = openCohorts.find((c) => c._id === currentCohort)
  ...
  useEffect(() => {
    ...
    const topObserver = new IntersectionObserver(([entry]) => setIsTopVisible(entry.isIntersecting), { threshold: 0.1 })

    const bottomObserver = new IntersectionObserver(([entry]) => setIsBottomVisible(entry.isIntersecting), {
      threshold: 0.1,
    })
  ...
  useEffect(() => {
    if (currentStatus) {
      const isExp = ["Working Professional", "Freelancer", "Business Owner", "Consultant"].includes(currentStatus)
      const expType = (
          ["Working Professional", "Business Owner", "Freelancer", "Consultant"].includes(currentStatus)
              ? currentStatus
              : ""
      ) as "" | "Working Professional" | "Business Owner" | "Freelancer" | "Consultant"
    ...
    const fetchStudentData = async () => {
      if (studentData._id && dataLoaded) {
        try {
          ...
          const studentDetail =
              student?.appliedCohorts[student.appliedCohorts.length - 1]?.applicationDetails?.studentDetails

          if (
              student?.appliedCohorts[student.appliedCohorts.length - 1]?.applicationDetails?.studentDetails !==
              undefined &&
              student?.appliedCohorts[student.appliedCohorts.length - 1]?.applicationDetails?.applicationStatus !==
              "incomplete"
          ) {
            setIsSaved(true)
          } else {
          ...
          if (
              student?.appliedCohorts[student.appliedCohorts.length - 1]?.applicationDetails?.applicationFee === "paid"
          ) {
          ...
          setApplicationFees(
              student?.appliedCohorts?.[student?.appliedCohorts.length - 1]?.cohortId?.cohortFeesDetail?.applicationFee,
          )
          ...
          const mergedForm = {
            studentData: {
              ...
              currentStatus:
                  ...
                  student?.appliedCohorts[student?.appliedCohorts.length - 1]?.qualification ||
                  "",
              courseOfInterest:
                  student?.appliedCohorts[student?.appliedCohorts.length - 1]?.status !== "dropped"
                      ? student?.appliedCohorts[student?.appliedCohorts.length - 1]?.cohortId?.programDetail?._id ||
                      existingData?.studentData?.courseOfInterest
                      : "",
              cohort:
                  student?.appliedCohorts[student?.appliedCohorts.length - 1]?.status !== "dropped"
                      ? student?.appliedCohorts[student?.appliedCohorts.length - 1]?.cohortId._id ||
            ...
            applicationData: {
              ...
              educationLevel:
                  ...
                  "",
              fieldOfStudy:
                  studentDetail?.previousEducation?.fieldOfStudy || existingData?.applicationData?.fieldOfStudy || "",
              institutionName:
              ...
              experienceType:
                  ...
                  (["Working Professional", "Business Owner", "Freelancer", "Consultant"].includes(
                      ...
                      : ""),
              nameOfCompany:
                  studentDetail?.workExperience?.nameOfCompany || existingData?.applicationData?.nameOfCompany || "",
              ...
              jobDescription:
                  studentDetail?.workExperience?.jobDescription || existingData?.applicationData?.jobDescription || "",
              emergencyFirstName:
                  studentDetail?.emergencyContact?.firstName || existingData?.applicationData?.emergencyFirstName || "",
              emergencyLastName:
                  studentDetail?.emergencyContact?.lastName || existingData?.applicationData?.emergencyLastName || "",
              emergencyContact:
                  studentDetail?.emergencyContact?.contactNumber || existingData?.applicationData?.emergencyContact || "",
              relationship:
              ...
              fatherOccupation:
                  ...
                  "",
              fatherEmail:
                  studentDetail?.parentInformation?.father?.email || existingData?.applicationData?.fatherEmail || "",
              motherFirstName:
              ...
              motherOccupation:
                  ...
                  "",
              motherEmail:
                  studentDetail?.parentInformation?.mother?.email || existingData?.applicationData?.motherEmail || "",
  ...
  const handlePayment = async () => {
    try {
      ...
      const feePayLoad = {
        ...
        cohortId: fetchedStudentData.appliedCohorts[fetchedStudentData.appliedCohorts.length - 1].cohortId._id,
      }
  ...
  const submitData = async (data: FormData, isSubmit?: boolean) => {
    const apiPayload = {
      ...
      studentDetailId:
      fetchedStudentData?.appliedCohorts[fetchedStudentData?.appliedCohorts.length - 1]?.applicationDetails
          ?.studentDetails?._id,
      studentData: {
  ...
  const onSubmit = async (data: FormData) => {
    console.log("Submitting form:", fetchedStudentData?.applicationDetails, isSaved)
      ...
      <>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6 mt-8">
                      ...
                      render={({ field }) => (
                          <FormItem className="flex-1 space-y-1">
                            ...
                            <FormControl>
                              <Input
                                  id="fullName"
                                  value={(fetchedStudentData?.firstName || "") + " " + (fetchedStudentData?.lastName || "")}
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1 relative">
                              ...
                              <FormControl>
                                <Input
                                    disabled={isSaved || fetchedStudentData?.isMobileVerified}
                                    id="contact"
                                    type="tel"
                              ...
                              {fetchedStudentData?.isMobileVerified ? (
                                  ...
                                  <Button
                                      ...
                                      disabled={otpLoading}
                                      onClick={() => handleVerifyClick(field.value || fetchedStudentData?.mobileNumber)}
                                      type="button"
                                  >
                  ...
                  <FormField
                      ...
                      render={({ field }) => (
                          <FormItem className="flex-1 space-y-1">
                            ...
                            <FormControl>
                              <Select disabled={isSaved} onValueChange={handleCourseChange} value={field.value}>
                                <SelectTrigger>
                                  <SelectValue placeholder="Select a Program" />
                                </SelectTrigger>
                                <SelectContent>
                                  {fetchedStudentData?.appliedCohorts?.[fetchedStudentData?.appliedCohorts.length - 1]?.cohortId
                                      ?.programDetail?._id && (
                                      <SelectItem
                                          key={
                      ...
                      render={({ field }) => (
                          <FormItem className="flex-1 space-y-1">
                            ...
                            <FormControl>
                              ...
                              >
                                <SelectTrigger>
                                  <SelectValue placeholder={!selectedProgram ? "Select a course first" : "Select"} />
                                </SelectTrigger>
                                <SelectContent>
                                  {fetchedStudentData?.appliedCohorts?.[fetchedStudentData?.appliedCohorts.length - 1]?.cohortId
                                          ?._id &&
                                      fetchedStudentData?.appliedCohorts?.[fetchedStudentData?.appliedCohorts.length - 1]
                                          ?.cohortId?.programDetail?._id === selectedProgram && (
                                          <SelectItem
                                              key={
                                                fetchedStudentData?.appliedCohorts?.[fetchedStudentData?.appliedCohorts.length - 1]
                                                    ?.cohortId._id
                ...
                <div className="flex flex-col sm:flex-row gap-4 sm:gap-2">
                  <FormField
                      ...
                      render={({ field }) => (
                          <FormItem className="flex-1 space-y-1 relative">
                            <Label className="text-sm font-normal pl-3">Your LinkedIn Profile Link (Optional)</Label>
                            <FormControl>
                              <Input
                                  className="pr-12"
                                  id="linkedInUrl"
                  ...
                  <FormField
                      ...
                      render={({ field }) => (
                          <FormItem className="flex-1 space-y-1 relative">
                            <Label className="text-sm font-normal pl-3">Your Instagram ID (Optional)</Label>
                            <FormControl>
                              <Input
                                  className="pr-12"
                                  id="instagramUrl"
                ...
                <div className="flex flex-col gap-4 sm:gap-6">
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-2">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1">
                              <Label htmlFor="educationLevel" className="text-sm font-normal pl-3">
                                Highest Level of Education Attained
                              </Label>
                              <FormControl>
                                <Select disabled={isSaved} onValueChange={field.onChange} value={field.value}>
                                  <SelectTrigger>
                                    <SelectValue placeholder="Select" />
                                  </SelectTrigger>
                                  <SelectContent>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1">
                              <Label htmlFor="fieldOfStudy" className="text-sm font-normal pl-3">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-2">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 flex flex-col space-y-1 relative">
                              <Label htmlFor="institutionName" className="text-sm font-normal pl-3">
                                Name of Institution
                              </Label>
                              <FormControl>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 flex flex-col space-y-1">
                              <Label htmlFor="graduationYear" className="text-sm font-normal pl-3">
                                Year of Graduation
                              </Label>
                              <FormControl>
                                <GraduationSelector
                              ...
                              <FormControl>
                                ...
                                >
                                  <div className="flex items-center space-x-2">
                                    ...
                                    <Label htmlFor="yesWorkExperience" className="text-base font-normal">
                                      Yes
                                    </Label>
                                  </div>
                                  <div className="flex items-center space-x-2">
                                    <RadioGroupItem value="no" id="noWorkExperience" />
                                    <Label htmlFor="noWorkExperience" className="text-base font-normal">
                                      No
                                    </Label>
                                  </div>
                                </RadioGroup>
                      ...
                      <>
                        <div className="flex flex-col sm:flex-row gap-4 sm:gap-2">
                          <FormField
                              ...
                              render={({ field }) => (
                                  <FormItem className="flex-1 space-y-1">
                                    <Label htmlFor="experienceType" className="text-sm font-normal pl-3">
                          ...
                          <FormField
                              ...
                              render={({ field }) => (
                                  <FormItem className="flex-1 space-y-1">
                                    <Label htmlFor="jobDescription" className="text-sm font-normal pl-3">
                        ...
                        {watchExperienceType === "Working Professional" && (
                            <div className="flex flex-col sm:flex-row gap-4 sm:gap-2">
                              <FormField
                                  ...
                                  render={({ field }) => (
                                      <FormItem className="flex-1 space-y-1">
                                        <Label htmlFor="companyName" className="text-sm font-normal pl-3">
                              ...
                              <div className="flex-1 space-y-1">
                                <Label htmlFor="duration" className="text-sm font-normal pl-3">
                                ...
                                <div className="grid sm:flex flex-1 items-center gap-4 sm:gap-2">
                                  <FormField
                                      ...
                                      {errors.applicationData.durationFrom.message}
                                    </span>
                                              )}
                                            </FormMessage>
                                          </FormItem>
                              ...
                              <FormField
                                  ...
                                  render={({ field }) => (
                                      <FormItem className="flex-1 flex flex-col">
                                        <Label htmlFor="companyStartDate" className="text-sm font-normal pl-3">
                                        ...
                                        <FormControl>
                                          <FormControl>
                                            <DurationSelector
                                                ...
                                                value={field.value}
                                                onChange={field.onChange}
                                                maxDate={new Date().toISOString().split("T")[0]} // YYYY-MM-DD
                                                disabled={isSaved}
                                            />
                                          </FormControl>
                                        </FormControl>
                ...
                <div className="flex flex-col gap-4 sm:gap-6">
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-2">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1">
                              <Label htmlFor="emergencyFirstName" className="text-sm font-normal pl-3">
                                First Name
                              </Label>
                              <FormControl>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1">
                              <Label htmlFor="emergencyLastName" className="text-sm font-normal pl-3">
                                Last Name
                              </Label>
                              <FormControl>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-2">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1 relative">
                              <Label htmlFor="emergencyContact" className="text-sm font-normal pl-3">
                                Contact No.
                              </Label>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1">
                              <Label htmlFor="relationship" className="text-sm font-normal pl-3">
                ...
                <div className="flex flex-col gap-4 sm:gap-6">
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-2">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1">
                              <Label htmlFor="fatherFirstName" className="text-sm font-normal pl-3">
                                Father&apos;s First Name
                              </Label>
                              <FormControl>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1">
                              <Label htmlFor="fatherLastName" className="text-sm font-normal pl-3">
                                Father&apos;s Last Name
                              </Label>
                              <FormControl>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-2">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1 relative">
                              <Label htmlFor="fatherContact" className="text-sm font-normal pl-3">
                                Father&apos;s Contact No.
                              </Label>
                              <div className="absolute left-3 top-[40.5px]">+91</div>
                              <FormControl>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1">
                              <Label htmlFor="fatherOccupation" className="text-sm font-normal pl-3">
                                Father&apos;s Occupation
                              </Label>
                              <FormControl>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-2">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1">
                              <Label htmlFor="fatherEmail" className="text-sm font-normal pl-3">
                                Father&apos;s Email
                              </Label>
                              <FormControl>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1">
                              <Label htmlFor="motherFirstName" className="text-sm font-normal pl-3">
                                Mother&apos;s First Name
                              </Label>
                              <FormControl>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-2">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1">
                              <Label htmlFor="motherLastName" className="text-sm font-normal pl-3">
                                Mother&apos;s Last Name
                              </Label>
                              <FormControl>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1 relative">
                              <Label htmlFor="motherContact" className="text-sm font-normal pl-3">
                                Mother&apos;s Contact No.
                              </Label>
                              <div className="absolute left-3 top-[40.5px]">+91</div>
                              <FormControl>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-2">
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1">
                              <Label htmlFor="motherOccupation" className="text-sm font-normal pl-3">
                                Mother&apos;s Occupation
                              </Label>
                              <FormControl>
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem className="flex-1 space-y-1">
                              <Label htmlFor="motherEmail" className="text-sm font-normal pl-3">
                                Mother&apos;s Email
                              </Label>
                              <FormControl>
                            ...
                            <FormControl>
                              ...
                              >
                                <div className="flex items-center space-x-2">
                                  ...
                                  <Label htmlFor="yesFinanciallyDependent" className="text-base font-normal">
                                    Yes
                                  </Label>
                                </div>
                                <div className="flex items-center space-x-2">
                                  <RadioGroupItem value="no" id="noFinanciallyDependent" />
                                  <Label htmlFor="noFinanciallyDependent" className="text-base font-normal">
                                    No
                                  </Label>
                                </div>
                              </RadioGroup>
                            ...
                            <FormControl>
                              ...
                              >
                                <div className="flex items-center space-x-2">
                                  ...
                                  <Label htmlFor="yesFinancialAid" className="text-base font-normal">
                                    Yes
                                  </Label>
                                </div>
                                <div className="flex items-center space-x-2">
                                  <RadioGroupItem value="no" id="noFinancialAid" />
                                  <Label htmlFor="noFinancialAid" className="text-base font-normal">
                                    No
                                  </Label>
                                </div>
                              </RadioGroup>
        ...
        <Dialog open={isPaymentDialogOpen}>
          ...
          <DialogContent className="max-h-[70vh] sm:max-h-[90vh] overflow-y-auto max-w-[90vw] sm:max-w-[500px] mx-auto bg-[#1C1C1C] text-white rounded-lg px-8 py-16 text-center shadow-[0px_4px_32px_0px_rgba(0,0,0,0.75)]">
            <img src="/assets/images/make-payment.svg" className="mx-auto mb-8" alt="payment" />
            <div>
              ...
              <div className="mt-2 text-xs sm:text-sm font-normal text-center">
                Make an admission fee payment of INR ₹{applicationFees || 0}.00 to move to the next step of your admission
                process
              </div>
            </div>
            <div className="flex flex-col gap-3">
              <Button
                  ...
                  disabled={loading}
              >
                <div className="flex items-center gap-2">{loading ? "Initializing Payment..." : "Make Payment"}</div>
              </Button>
            </div>
          </DialogContent>
        </Dialog>
